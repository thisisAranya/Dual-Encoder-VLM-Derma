"""
train.py - Training script for Dual Vision Encoder
"""
...
class DualVisionTrainer:
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch in pbar:
            ...
            try:
                ...
                # Calculate loss (you might need to implement custom loss)
                ...
                if outputs['has_images']:
                    total_fusion_weights['qwen'] += outputs['fusion_weight_qwen']
                    total_fusion_weights['dinov2'] += outputs['fusion_weight_dinov2']
    ...
    def calculate_loss(self, outputs: Dict, batch: Dict) -> torch.Tensor:
        """
        Calculate training loss
        This is a placeholder - you'll need to implement based on your specific task
        """
        # For now, return a dummy loss
        # In practice, you might compare generated text with ground truth
        # or use other task-specific losses
        
        if outputs['has_images']:
            # Could add regularization on fusion weights
            fusion_regularization = torch.tensor(0.0, device=self.device)
            alpha = outputs['alpha']
            
            # Encourage diverse usage of both encoders
            # Penalize if alpha is too close to 0 or 1
            diversity_loss = -torch.mean(alpha * torch.log(alpha + 1e-8) + 
                                       (1 - alpha) * torch.log(1 - alpha + 1e-8))
            
            fusion_regularization += self.config['diversity_loss_weight'] * diversity_loss
            
            return fusion_regularization
        else:
